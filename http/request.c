...
static void initParams(char *paramsStr,dlhash *dlhash){
    ...
    do {
        ...
        char *data = (char *)malloc((strlen(paramsTmp->head->data) + 1)*sizeof(char));
        strcpy(key,urldecode(paramsTmp->tail->data));
        strcpy(data,urldecode(paramsTmp->head->data));
        value->key  = key;
        value->data = data;
        dlhashInsert(dlhash,value);
        _dlistDestroy(paramsTmp);
        _free(paramsTmp);
